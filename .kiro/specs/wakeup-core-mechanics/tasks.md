...
> ### 复选框语义（PT-03 对齐，2026-08-09）
>
> 承上方 D-062 冻结提示。本次按 `docs/并行作战/PT-03_复选框对齐现实.md` 给每一项补上可核对的状态注记。
> 因 `core-mechanics/` 全线零测试且 D-062 冻结，**没有任何任务达到 `[x]`（代码存在且有测试证明其正确/可装载）**，
> 全部保持 `[ ]`，并按两态注记：
>
> | 标记 | 含义 |
> |---|---|
> | `[ ]` + `（待测：代码已存在 …）` | 对应 `.ts` 源码已存在，但零测试证明其正确性/可装载性；路径均经 `Test-Path` 校验存在。 |
> | `[ ]` + `（未做：…）` | 目标实现或文件不存在（含 41 条必交付属性测试、rules.stamina/turnround、actions.window 等）。 |
>
> 特别地：属性测试任务（9.2、10.1–10.41）与检查点（5、8）一律 `[ ]`，成因即上方 D-062 冻结 + 零测试。
> 可信的"完成"判定需等 D-062 收敛执行（并行任务 PT-07，🔒 未就绪）落地后重新评估。本次不改任何 `src/**` 代码。


- [ ] 1. 玩法层数值归属与装载期治理基座 （待测：代码已存在 src/play/core-mechanics/ownership.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  - [ ] 1.1 建立 `src/play/core-mechanics/ownership.ts` 的归属类型与数值字段遍历 （待测：代码已存在 src/play/core-mechanics/ownership.ts（collectNumericFields / NumericOwnership 等类型）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 1.2 在 `src/play/core-mechanics/ownership.ts` 实现归属分类校验与可见值域纯函数 （待测：代码已存在 src/play/core-mechanics/ownership.ts（validateNumericOwnership / validateGameplayValueRange / isVisibleGameplayValue / assertInternalNotInProjectionWhitelist）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 1.3 在 `src/play/core-mechanics/ownership.ts` 实现术语、废案、来源追踪与未冻结项校验 （待测：代码已存在 src/play/core-mechanics/ownership.ts（validateTerminology / validateNotDeprecated / validateProvenance / validateUnresolvedGuards / collectBlockedCapabilities）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
...
- [ ] 2. 玩法层纯函数（无随机、无 WorldState） （待测：代码已存在 src/play/core-mechanics/allocation.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 2.2 在 `src/play/core-mechanics/allocation.ts` 实现顺序键比较与较长剩余选择 （待测：代码已存在 src/play/core-mechanics/allocation.ts（compareTurnOrder / pickLongerRemainingTurns / validateVisibleRange）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
...
- [ ] 3. 声明式定义集合（`src/play/core-mechanics/defs/`） （待测：代码已存在 src/play/core-mechanics/defs/；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  - [ ] 3.1 编写 `src/play/core-mechanics/defs/attachments.ts` （待测：代码已存在 src/play/core-mechanics/defs/attachments.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 3.2 编写 `src/play/core-mechanics/defs/schedule.ts` （待测：代码已存在 src/play/core-mechanics/defs/schedule.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 3.3 编写 `src/play/core-mechanics/defs/actions.paid.ts` （待测：代码已存在 src/play/core-mechanics/defs/actions.paid.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 3.4 编写 `src/play/core-mechanics/defs/actions.attached.ts` （待测：代码已存在 src/play/core-mechanics/defs/actions.attached.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 3.5 编写 `src/play/core-mechanics/defs/rules.damage.ts` （待测：代码已存在 src/play/core-mechanics/defs/rules.damage.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 3.6 编写 `src/play/core-mechanics/defs/rules.status.ts` （待测：代码已存在 src/play/core-mechanics/defs/rules.status.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 3.7 编写 `src/play/core-mechanics/defs/rules.phase.ts` （待测：代码已存在 src/play/core-mechanics/defs/rules.phase.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 3.8 编写 `src/play/core-mechanics/defs/rules.gateway.ts` （待测：代码已存在 src/play/core-mechanics/defs/rules.gateway.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
...
- [ ] 4. 玩法包、装载入口与玩法层 Linter （待测：代码已存在 src/play/core-mechanics/load.ts；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  - [ ] 4.1 编写 `src/play/core-mechanics/playpack.ts` （待测：代码已存在 src/play/core-mechanics/defs/playpack.ts（CoreMechanicsPlaypack）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 4.2 在 `src/play/core-mechanics/load.ts` 实现装载入口与组合根接线 （待测：代码已存在 src/play/core-mechanics/load.ts（loadCoreMechanics）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 4.3 在 `src/play/core-mechanics/load.ts` 实现玩法层 Linter 全量规则 （待测：代码已存在 src/play/core-mechanics/load.ts（玩法层 Linter 全量 E_LOAD_* 规则）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 4.4 在 `src/play/core-mechanics/load.ts` 实现拒绝原因映射与统一提交入口 （待测：代码已存在 src/play/core-mechanics/load.ts（PLAY_REJECTION_TO_ERRCODE / CoreMechanicsFacade）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 4.5 实现 design.md 11.6 的「字段缺失被当成 0」防护 （待测：代码已存在 src/play/core-mechanics/defs/rules.damage.ts, src/play/core-mechanics/defs/actions.attached.ts, src/play/core-mechanics/defs/rules.phase.ts（字段缺失被当成 0 的防护）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
...
- [ ] 5. 检查点 - 装载链路可编译可装载 （未做：检查点要求"所有测试通过"，但 src/play/core-mechanics/ 下零测试文件，无可运行的检查点；D-062 冻结中）
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. 过载机制（design 3.17，D-055；建立在 AP/体力/阶段任务 1–4 之上） （未做：过载运行期实现基本缺失：过载态 AttachmentDef 未落 src/play/core-mechanics/defs/attachments.ts、rules.stamina.ts 不存在、归队计数未落 rules.phase.ts；仅 load.ts/ownership.ts 有 status_overloaded 校验字符串；D-062 冻结中）
  - [ ] 6.1 在 `src/play/core-mechanics/defs/attachments.ts` 追加过载态 `AttachmentDef` （未做：src/play/core-mechanics/defs/attachments.ts 内无过载态 AttachmentDef（status_overloaded 仅作为校验标识出现在 src/play/core-mechanics/load.ts 与 src/play/core-mechanics/ownership.ts，未在 attachments 定义集中声明）；D-062 冻结中）
  ...
  - [ ] 6.2 新增 `src/play/core-mechanics/defs/rules.stamina.ts` 承载 `play.stamina.grant` 过载触发 （未做：目标文件 src/play/core-mechanics/defs/rules.stamina.ts 不存在（play.stamina.grant 过载触发规则未实现）；D-062 冻结中）
  ...
  - [ ] 6.3 在 `src/play/core-mechanics/defs/rules.phase.ts` 追加归队计数与失权推进 （未做：src/play/core-mechanics/defs/rules.phase.ts 内无归队计数与失权推进（overloadRejoinPending 仅出现在 src/play/core-mechanics/load.ts 的校验分支，未落相位规则）；D-062 冻结中）
  ...
  - [ ] 6.4 在 `src/play/core-mechanics/load.ts` 追加过载装载期校验（Requirement 16.9 清单） （待测：代码已存在 src/play/core-mechanics/load.ts（含 status_overloaded 过载装载期校验分支）；仅装载期校验存在，且无测试；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
...
- [ ] 7. 行动轮排名与窗口期（design 3.18，D-035/D-036/D-053/D-055；建立在阶段任务 3.7 与过载任务 6 之上） （未做：行动轮排名 / 窗口期机制的运行期定义文件全部缺失（rules.turnround.ts、actions.window.ts 不存在）；逆转/超逆转仅作为标识符出现在 src/play/core-mechanics/defs/ids.ts；D-062 冻结中）
  - [ ] 7.1 新增 `src/play/core-mechanics/defs/rules.turnround.ts` 承载排名交换与推进扫描 （未做：目标文件 src/play/core-mechanics/defs/rules.turnround.ts 不存在（排名交换与推进扫描未实现）；D-062 冻结中）
  ...
  - [ ] 7.2 新增 `src/play/core-mechanics/defs/actions.window.ts` 承载逆转/超逆转窗口期动作 （未做：目标文件 src/play/core-mechanics/defs/actions.window.ts 不存在（逆转/超逆转窗口期动作未实现；仅 src/play/core-mechanics/defs/ids.ts 有"逆转/超逆转"标识符）；D-062 冻结中）
  ...
  - [ ] 7.3 在 `src/play/core-mechanics/defs/rules.phase.ts` 追加窗口期提交的成本结算与延迟兑现 （未做：src/play/core-mechanics/defs/rules.phase.ts 内无窗口期提交的成本结算与延迟兑现实现；D-062 冻结中）
  ...
  - [ ] 7.4 在 `src/play/core-mechanics/load.ts` 追加行动轮/窗口期装载期校验 （未做：src/play/core-mechanics/load.ts 内无行动轮/窗口期装载期校验规则；D-062 冻结中）
...
- [ ] 8. 检查点 - 过载与行动轮定义可编译可装载 （未做：检查点要求"所有测试通过"，但 src/play/core-mechanics/ 下零测试文件；D-062 冻结中）
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. 属性测试生成器与投影层（属性测试的前置依赖） （未做：投影层 src/play/core-mechanics/projection.ts 已存在（见 9.1），但属性测试生成器（9.2）缺失且全线零测试；D-062 冻结中）
  - [ ] 9.1 在 `src/play/core-mechanics/projection.ts` 实现只读投影 （待测：代码已存在 src/play/core-mechanics/projection.ts（ProjectedResources 只读投影）；D-062 冻结 + core-mechanics 目录零测试（无 __tests__/），正确性与可装载性未被任何断言验证）
  ...
  - [ ] 9.2 在 `src/play/core-mechanics/__tests__/property/generators.ts` 建立共享 fast-check 生成器 （未做：目标文件 src/play/core-mechanics/__tests__/property/generators.ts 不存在；src/play/core-mechanics/ 下无任何 __tests__ 目录；D-062 冻结中）
...
- [ ] 10. 属性测试（一文件一属性，文件名含属性编号，`fast-check` `numRuns` ≥ 100；全部为必交付项） （未做：本 Spec"必交付"的 41 条属性测试一条也没有写（src/play/core-mechanics/__tests__/property/ 不存在）；D-062 冻结中）
  - [ ] 10.1 `__tests__/property/p01-op-set-invariance.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p01-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 1: 状态变化只经已登记 Op 与唯一写入通道`
    - _需求：2.1, 13.7, 15.8, 19.3_
  - [ ] 10.2 `__tests__/property/p02-rejection-preserves-state.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p02-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 2: 拒绝保持事务前状态`
    - _需求：2.3, 2.4, 2.5, 4.9, 16.4_
  - [ ] 10.3 `__tests__/property/p03-visible-values-1-5.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p03-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 3: 玩家可见数值恒在 1-5 且不出现 0`
    - _需求：3.1, 3.2, 3.3, 11.1_
  - [ ] 10.4 `__tests__/property/p04-depletion-discrete-projection.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p04-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 4: 资源耗尽与无独立成本投影为离散取值`
    - _需求：3.4, 3.6, 5.7_
  - [ ] 10.5 `__tests__/property/p05-parallel-options-max-5.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p05-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 5: 同时并列独立选项不超过 5`
    - _需求：3.8, 8.7, 12.10_
  - [ ] 10.6 `__tests__/property/p06-unclassified-numeric-rejected.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p06-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 6: 未分类数值拒绝装载`
    - _需求：3.7, 3.9_
  - [ ] 10.7 `__tests__/property/p07-paid-action-one-ap.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p07-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 7: 付费动作恰好 1 AP，不存在 2 AP 原子动作`
    - _需求：4.2, 4.3, 4.7, 9.6_
  - [ ] 10.8 `__tests__/property/p08-attached-never-toplevel.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p08-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 8: 附着动作永不成为顶层分支`
    - _需求：4.8, 8.4, 8.5, 8.8_
  - [ ] 10.9 `__tests__/property/p09-parent-failure-zero-write.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p09-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 9: 父动作失败则附着效果零写入`
    - _需求：8.6_
  - [ ] 10.10 `__tests__/property/p10-ap-differential-allocation.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p10-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 10: AP 差值分配表`
    - _需求：5.4, 5.5, 5.6, 5.7, 5.12_
  - [ ] 10.11 `__tests__/property/p11-two-player-no-3ap.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p11-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
  ...
  - [ ] 10.14 `__tests__/property/p14-deterministic-replay.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p14-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    ...
    - _需求：5.10, 16.6_
  - [ ] 10.15 `__tests__/property/p15-stamina-cap-5.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p15-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 15: 体力上限恒为 5 且不触发未冻结过载`
    - _需求：6.1, 6.2, 6.14_
  - [ ] 10.16 `__tests__/property/p16-boost-stamina-conservation.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p16-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 16: 强力骰体力守恒`
    - _需求：6.8, 6.9_
  - [ ] 10.17 `__tests__/property/p17-boost-two-tiers.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p17-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 17: 强力骰仅两档、承诺不可撤销、不足不部分冻结`
    - _需求：6.3, 6.4, 6.5, 6.6_
  - [ ] 10.18 `__tests__/property/p18-five-phase-order-turnorder-fixed.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p18-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 18: 五阶段顺序与结算后固定的行动顺序`
    - _需求：7.1, 7.3, 7.6_
  - [ ] 10.19 `__tests__/property/p19-phase-guard-cleanup-atomic.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p19-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 19: 阶段推进守卫与清理阶段原子性`
    - _需求：7.9, 7.10_
  - [ ] 10.20 `__tests__/property/p20-timing-purity.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p20-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 20: 时序纯洁性`
    - _需求：8.1, 8.2_
  - [ ] 10.21 `__tests__/property/p21-precise-two-step-interrupt.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p21-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 21: 精密交互的两步结构与中断语义`
    - _需求：9.1, 9.2, 9.3, 9.4, 9.7_
  - [ ] 10.22 `__tests__/property/p22-gateway-all-or-nothing.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p22-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 22: 网关全成或全不成`
    - _需求：10.3, 10.5, 10.6, 10.7_
  - [ ] 10.23 `__tests__/property/p23-downed-atomic-no-zero.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p23-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 23: 零血倒地的原子转换且从不暴露 0`
    - _需求：3.5, 11.3_
  - [ ] 10.24 `__tests__/property/p24-downed-action-set-exit-irreversible.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p24-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 24: 零血倒地动作集不扩大、退出不可逆`
    - _需求：11.4, 11.5, 11.6, 11.7_
  - [ ] 10.25 `__tests__/property/p25-heal-cap-no-revive.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p25-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 25: 治疗上限与不复活`
    - _需求：11.8, 15.2_
  - [ ] 10.26 `__tests__/property/p26-eternal-sleep-three-atomic.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p26-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 26: 令其长眠的三事原子性`
    - _需求：6.10, 12.5, 12.6, 12.11_
  - [ ] 10.27 `__tests__/property/p27-death-bag-conservation.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p27-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 27: 死亡背包物品守恒、只出不进、容量派生`
    - _需求：12.7, 12.8, 12.9_
  - [ ] 10.28 `__tests__/property/p28-status-expiry-no-zero.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p28-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 28: 状态到期不保留可见 0`
    - _需求：13.1, 13.4_
  - [ ] 10.29 `__tests__/property/p29-refresh-longer-remaining.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p29-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 29: 刷新策略保留较长剩余时间且不叠加强度`
    - _需求：13.2_
  - [ ] 10.30 `__tests__/property/p30-status-config-required.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p30-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 30: 状态配置缺必需项即拒绝`
    - _需求：13.3, 13.8_
  - [ ] 10.31 `__tests__/property/p31-block-lifecycle.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p31-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 31: 格挡生命周期`
    - _需求：14.1, 14.2, 14.3_
  - [ ] 10.32 `__tests__/property/p32-stealth-scene-move-find.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p32-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 32: 隐蔽的场景限定、移动移除与不可被找到`
    - _需求：14.5, 14.6, 14.7_
  - [ ] 10.33 `__tests__/property/p33-no-implicit-recovery.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p33-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 33: 无隐式恢复，增量必可归因`
    - _需求：6.12, 6.13, 15.1, 15.3, 15.7_
  - [ ] 10.34 `__tests__/property/p34-sleep-two-step.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p34-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 34: 睡眠两步流程与起床回满`
    - _需求：6.11, 15.4_
  - [ ] 10.35 `__tests__/property/p35-same-legality-all-sources.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p35-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 35: UI、AI、UGC 与玩家共用同一合法性判定与同一拒绝原因`
    - _需求：4.5, 16.7, 18.2, 19.6_
  - [ ] 10.36 `__tests__/property/p36-load-atomic-reject-presentation-fallback.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p36-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 36: 装载的原子拒绝与表现字段降级边界`
    - _需求：16.1, 16.2, 16.3_
  - [ ] 10.37 `__tests__/property/p37-unresolved-reject-closed-accept.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p37-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 37: 引用未冻结项一律拒绝且不产生默认值`
    - _需求：8.9, 11.2, 14.8, 16.8, 17.1, 17.2, 17.3_
  - [ ] 10.38 `__tests__/property/p38-deprecated-not-revived.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p38-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 38: 已否决机制不得复活`
    - _需求：1.5, 13.6_
  - [ ] 10.39 `__tests__/property/p39-layer-ownership-provenance-conflict.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p39-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 39: 层级归属、来源状态与冲突保留`
    - _需求：1.1, 1.2, 1.3, 1.4, 1.6, 1.7_
  - [ ] 10.40 `__tests__/property/p40-normal-downed-explicit.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p40-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
    - `// Feature: wakeup-core-mechanics, Property 40: 普通倒地的显式触发与场景约束`
    - _需求：12.1, 12.2, 12.3, 12.4_
  - [ ] 10.41 `__tests__/property/p41-npc-order-budget.test.ts` （未做：属性测试文件 src/play/core-mechanics/__tests__/property/p41-*.test.ts 不存在（41 条必交付属性测试全部未写）；D-062 冻结中）
