...
具体枪械伤害数值、掩体减伤数值、出生规则、胜负条件、缩圈流程、地图布局、NPC 个体行为参数、具体物品实例配置、具体网关门槛、UI 布局与动画、网络传输格式。以及 `requirements.md` Requirement 17 列出的仍未冻结项 **T-001、T-002（仅数值）、U-001** —— 本设计只为它们留出接口与阻塞点，见第 9 章。

> **已裁决关闭（不再是阻塞点）**：U-002（D-037 单人得 2 AP）、**U-003（D-055 过载取得规范位阶）**、U-004（D-053 六项机制批准）、U-005（D-052 NPC 默认 1 AP / SP 上限 3）、T-002 的结构部分（D-040 二维正交模型）。第 9 章对应小节保留为**裁决记录**，不再是留白。
...
/**
 * 仍未冻结的项。U-002（D-037）、**U-003（D-055）**、U-004（D-053）、U-005（D-052）已裁决关闭并移出；
 * T-002 收窄为仅"掩体数值"未冻结，其结构已由 D-040 冻结。
 * 因此本联合类型恰好剩三个成员：T-001、T-002（仅数值）、U-001。
 */
...
### Property 13: 单一投点参与者阻塞而非推断默认

*对于任意*只有 1 名投点参与者的投点局面，标准投点结算都应返回结构化拒绝并标记为阻塞，不应写入任何 AP 分配结果。
...
### 9.3 U-001 基础投点生成与强力骰越界策略

- **不得决定**：1-5 基础等级的分布（不得把历史 d6 改写为均匀 1-5、映射表或任何其他分布），以及 +1/+2 修正结果越过 5 时的处理（不得默认截断、回绕、重投或拒绝承诺）。
- **开放接口**：`RollPolicyBinding`（3.3 节），两个策略引用 + 一个开关。
- **激活时的拒绝**：`enableRandomRoll === true` 且任一引用为 null → 装载期 `E_LOAD_UNRESOLVED_CONTRACT`，`reason = 'U-001'`；运行期守卫在任何随机流推进与体力扣减之前 `abort`。
- **不阻塞**：AP 差值分配算法（以外部提供的合法最终等级验证，Property 10、11）。

### 9.4 U-002 单一投点参与者的 AP（已由 D-037 裁决关闭）

- **裁决结论（D-037）**：投点 AP 分配**严格按既有算法执行**，不因参与人数少而改用特例。唯一的人数相关处理是**按场上人数裁剪档位**：
...
- **不再拒绝**：标准配置中可能只剩一名投点参与者的情形，装载结果中**不再登记** U-002 阻塞。
...
- **信息可见性的三档裁决（D-031 / D-032 / D-033）**，这部分不再是未冻结内容：
  ...
  | **低显著性公开** | `[瞄准中]` | 悬停显示红点线，**无头顶感叹号**（D-033） |
...
| 3.5 五回合阶段 | 五阶段：投点 → 结算 → 玩家行动 → NPC 行动 → 清理 | 三相：投点 → 行动 → 清理（`ScheduleDef.phases` + `roundEnd`） | **结构冲突**：本设计的"结算"与"NPC 行动"两个阶段在该包中不作为独立相位存在 |
| 3.5 行动顺序固定性 | 顺序表落在 `world.props.play.turnOrder`，结算阶段一次性写入，仅 D-053 六项机制可改写 | 队列落在 `world.props.actionTurn.queue`，行动阶段内由弱点/招架/处决命中即 `list.move` 立即改写 | **状态落点不同 + 改写时机不同**：两处状态互不同步，同时激活会产生两份行动轮状态 |
| 3.4 体力与强力骰 | 上限 5、两档承诺（1 体力 +1 级 / 2 体力 +2 级）、不可撤销、退还与结算互斥；**过载是标准默认规则（D-055）**，引用 `status_overloaded` 合法 | 强力骰 ±1/±2 有成本与互斥；**过载已实现**（尝试使 SP 超过 5 即触发，含跳过本回合与恢复计数 `remainingRolls = 2`） | ~~直接冲突~~ **已消解（D-055）**：两侧方向一致，过载均为生效机制。剩余的持续时长数值分歧（"下下回合" vs `remainingRolls:2`）挂 L3-DIV-01，属数值未决 |
| 9.3 U-001 投点生成 | 基础等级分布未冻结，`enableRandomRoll === true` 且策略引用为 null 即拒绝装载；运行期守卫在任何随机流推进前 `abort` | 已实现 d5 投点（其 A-01 记为自主设计，理由是 d6 会产出 6、违反 1-5 铁律），并标注为"待裁决的规范冲突" | **冲突**：该包已选定一个分布，本设计按 U-001 禁止选定 |
