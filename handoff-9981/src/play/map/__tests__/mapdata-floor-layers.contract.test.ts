...
describe('canonical 序列化 / 解析 roundtrip', () => {
  ...
  it('旧 JSON 含 buildingGroups 时导入直接拒绝', () => {
    const legacyBuildingJson = JSON.stringify({
      ...canonicalMap(),
      buildingGroups: [],
    });
    expect(() => parseMapData(legacyBuildingJson)).toThrow('MAP_BUILDING_GROUP_DEPRECATED');
  });

  it('结构校验明确阻断旧 buildingGroups 字段', () => {
    const legacyBuildingMap = {
      ...canonicalMap(),
      buildingGroups: [],
    } as unknown as CanonicalMapData;
    const findings = validateMapStructure(legacyBuildingMap);
    expect(findings).toEqual(expect.arrayContaining([
      expect.objectContaining({ code: 'MAP_BUILDING_GROUP_DEPRECATED', severity: 'error' }),
    ]));
  });

  it('旧 v1 JSON 含 buildingGroups 时同样拒绝，不静默丢弃', () => {
    expect(() => parseMapData(JSON.stringify({ ...legacyMap(), buildingGroups: [] }))).toThrow(
      'MAP_BUILDING_GROUP_DEPRECATED',
    );
  });
