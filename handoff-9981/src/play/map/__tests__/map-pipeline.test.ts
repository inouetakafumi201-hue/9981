...
describe('地图校验：能证伪每条规则的反向用例', () => {
  ...
  it('每条诊断都有面向创作者的说明与改法，不只有错误码', () => {
    ...
    it('占位框未绑定 spriteRef 时给出 warning 诊断且不阻止发布', () => {
      const data = map({
        nodes: [node('a'), node('b')],
        edges: [edge('e1', 'a', 'b')],
        placeholderBoxes: [
      ...
      const findings = validateMapStructure(data);
      expect(codesOf(findings)).toContain('MAP_PLACEHOLDER_NO_SPRITE_REF');
      expect(canPublish(findings)).toBe(true);
    });
    ...
    it('占位框越界或无效坐标时拒绝', () => {
      ...
      const findings = validateMapStructure(data);
      expect(codesOf(findings)).toContain('MAP_PLACEHOLDER_BOUNDS_INVALID');
      expect(canPublish(findings)).toBe(false);
    });
    ...
    it('占位框重复 id 报错', () => {
      const data = map({
        nodes: [node('a')],
        placeholderBoxes: [
          { id: 'dup_box', type: 'asset', bounds: { x: 0.1, y: 0.1, width: 0.2, height: 0.2 } },
          { id: 'dup_box', type: 'asset', bounds: { x: 0.3, y: 0.3, width: 0.2, height: 0.2 } },
        ],
      });
      expect(codesOf(validateMapStructure(data))).toContain('MAP_DUPLICATE_PLACEHOLDER_ID');
    });
        ...
        placeholderBoxes: [
          {
            ...
            connectedEdgeId: 'non_existent_edge',
          },
        ],
      });
      expect(codesOf(validateMapStructure(data))).toContain('MAP_PLACEHOLDER_EDGE_NOT_FOUND');
    });

    it('已绑定 spriteRef 的占位框通过校验且无 warning', () => {
      const data = map({
        nodes: [node('a'), node('b')],
        edges: [edge('e1', 'a', 'b')],
        placeholderBoxes: [
