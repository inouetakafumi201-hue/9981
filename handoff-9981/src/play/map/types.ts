 ...
 *
 * 策划在地图编辑器中对需要交互的原生组件框选占位框。
 * 编辑器导出切片 crops/*.png，AI 以切片为母图进行图生图，
 * 生成后将素材 id 回填至 spriteRef，在游戏运行时 1:1 精确覆盖并实现无缝交互/过门。
 */
...
function clonePlaceholderBox(box: MapPlaceholderBox): MapPlaceholderBox {
  return {
    id: box.id,
    type: box.type,
    bounds: {
    ...
    ...(box.connectedEdgeId !== undefined ? { connectedEdgeId: box.connectedEdgeId } : {}),
    ...(box.hostNodeId !== undefined ? { hostNodeId: box.hostNodeId } : {}),
    ...(box.sourceCrop !== undefined ? { sourceCrop: box.sourceCrop } : {}),
    ...(box.spriteRef !== undefined ? { spriteRef: box.spriteRef } : {}),
  };
}

function normalizePlaceholderBoxes(
  boxes: readonly MapPlaceholderBox[] | undefined,
): readonly MapPlaceholderBox[] | undefined {
