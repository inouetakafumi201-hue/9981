...
function ordered(map: CanonicalMapData): Record<string, unknown> {
  return {
    ...
    edges: map.edges.map((edge) => ({ id: edge.id, def: edge.def, a: edge.a, b: edge.b, directionality: edge.directionality, ...(edge.interaction !== undefined ? { interaction: edge.interaction } : {}), path: edge.path.map((point) => ({ ...point })), ...(edge.visualObstruction !== undefined ? { visualObstruction: edge.visualObstruction } : {}), ...(edge.physicalObstruction !== undefined ? { physicalObstruction: edge.physicalObstruction } : {}), ...(edge.transitionWindow !== undefined ? { transitionWindow: edge.transitionWindow } : {}), ...(edge.semanticAnchor !== undefined ? { semanticAnchor: edge.semanticAnchor } : {}) })),
