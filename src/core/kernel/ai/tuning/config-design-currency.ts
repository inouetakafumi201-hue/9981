...
function nodeBuiltin<T>(name: string): T {
  const getter = (globalThis.process as typeof process & { getBuiltinModule?: (id: string) => unknown } | undefined)?.getBuiltinModule;
  if (getter === undefined) throw new Error(`${name} is not available in browser runtime`);
...
export function loadDesignCurrencyConfig(filePath: string): DesignCurrencyConfig {
  const fs = nodeBuiltin<NodeFileSystem>('node:fs');
...
export function saveDesignCurrencyConfig(filePath: string, config: DesignCurrencyConfig): void {
  ...
  if (separatorIndex > 0) fs.mkdirSync(filePath.slice(0, separatorIndex), { recursive: true });
  const payload = {
