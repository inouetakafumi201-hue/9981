 ...
 *
 * 版本：1.0.0（2026-08-11）
 * 迁出源：spec-compiler/messages.ts
 *
 * 职责：提供诊断消息的多语言翻译包。支持动态参数插值（{name} 占位符）。
 * 设计不变量：
 * - 诊断代码与消息内容分离（code 永不翻译）
 * - 参数保持结构化，不在编译期展开
 * - 缺失翻译自动降级为默认语言
 */
 ...
 *
 * 标题：简短观察，不使用编译器术语
 * 指导：后续步骤，可通过 {name} 引用 messageArgs
 */
...
/**
 * 完整消息包（BCP 47 locale + 条目字典）
 *
 * 作为"唯一生产创作者消息的地方"，此类型决定了诊断的本地化程度。
 * 宿主可通过注册多个包来支持多语言。
 */
 ...
 * 编译器可能产生的所有诊断代码
 *
 * 测试扫描编译器源码，当发现此处未列的代码时失败。
 * 这保证了没有未翻译的诊断会无声地漂向创作者。
 */
 ...
 * 需要参数插值的诊断代码与其参数合约
 *
 * 每个代码仅单一排放点，参数合约可通过代码审查验证。
 * 无 {name} 的指导消息确保遗漏参数不会泄露给创作者。
 */
...
/**
 * 简体中文消息包（DEFAULT_ZH_CN_BUNDLE）
 *
 * 完整消息目录。所有 COMPILER_EMITTED_CODES 代码均有对应条目。
 * 用于创作者界面、编译日志、IDE 诊断显示。
 */
...
export function unresolvedPlaceholders(text: string): string[] {
  return Array.from(text.matchAll(PLACEHOLDER)).map((match) => match[1] as string);
}
