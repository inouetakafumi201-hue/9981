...
export function BattleHud({
  ...
  variant?: HudVariant
  /**
   * 双轨制端到端接线 step 6：当提供时，使用真后端投影的 `cardActions` / `highlightActions`
  ...
  /** 0费池切页（未来扩展位）：'paid' = 主牌组，'zero' = 0费主动技能池 */
  const [zeroPoolTab, setZeroPoolTab] = useState<'paid' | 'zero'>('paid')
  ...
  const paidCards = useMemo<ActionCard[]>(() => {
    if (realActions !== undefined) {
      // 卡片轨动作：当前 core 包均为付费（1 AP）动作，作为主牌组
      return realActions.cardActions.map<ActionCard>((a) => ({
        id: a.actionId,
        name: a.accessibleLabel,
        description: a.effectText ?? a.accessibleLabel,
    ...
    return actionCards.filter((c) => c.cost > 0)
  }, [realActions])

  const freeCards = useMemo<ActionCard[]>(() => {
    if (realActions !== undefined) {
      // 0费主动技能池：当前 core 包尚无 track:card 的 0费 skill，预留扩展位
      return realActions.cardActions
        .filter((a) => a.costCategory === 'attached')
        .map<ActionCard>((a) => ({
          id: a.actionId,
          name: a.accessibleLabel,
          description: a.effectText ?? a.accessibleLabel,
  ...
  async function sendBattleIntent(intentId: Parameters<typeof createIntent>[0], payload: Record<string, unknown>): Promise<IntentStatus> {
    ...
    if (realActions !== undefined && (intentId === 'battle.select-action' || intentId === 'battle.select-target')) {
      if (intentId === 'battle.select-action') {
        const actionId = String(payload.actionId ?? '')
        const targetId = payload.targetId !== undefined ? String(payload.targetId) : undefined
        const accepted = realActions.submitAction(actionId, targetId !== undefined ? { targetId } : undefined)
        const status: IntentStatus = accepted ? 'accepted' : 'rejected'
        setIntentFeedback({ status, message: accepted ? `${actionId} accepted` : `${actionId} rejected` })
        return status
      }
      // battle.select-target: submit with targetId already in payload
      const actionId = String(payload.actionId ?? '')
      const targetId = String(payload.targetId ?? '')
      const accepted = realActions.submitAction(actionId, { targetId })
      const status: IntentStatus = accepted ? 'accepted' : 'rejected'
      setIntentFeedback({ status, message: accepted ? `target ${targetId} accepted` : `target ${targetId} rejected` })
      return status
    }
  ...
  return (
    <div className={`battle-hud bh-variant-${variant.toLowerCase()}`}>
      ...
      {/* ACTION HAND — a Kards-style fanned hand of cards, bottom-right. Hover lifts
        ...
        {/*
         * 0费主动技能池切页按钮（双轨制 P1-B1）：底栏左侧固定入口。
         * 0费主动技能是「甜品操作」机制定位，core 包当前尚无 track:card 0费 skill，
         * 此处渲染是「扩展位」——按钮就绪，0费池与主牌组通过 zeroPoolTab 互斥显示。
         */}
        {realActions !== undefined && (
          <button
            type="button"
            className={`bh-zero-pool-toggle ${zeroPoolTab === 'zero' ? 'is-active' : ''}`}
            aria-pressed={zeroPoolTab === 'zero'}
            onClick={() => setZeroPoolTab((t) => (t === 'paid' ? 'zero' : 'paid'))}
            title="切换 0费主动技能池"
          >
            <span className="bh-zero-pool-toggle-label">0费</span>
          </button>
        )}
        <div className="bh-hand-free" role="list" aria-label="Free actions">
          {zeroPoolTab === 'zero' ? (
            <div className="bh-zero-pool-empty" role="status">
              <span>暂无可用 0费主动技能</span>
              <small>未来扩展位（双轨制 P1-A2）</small>
            </div>
          ) : null}
