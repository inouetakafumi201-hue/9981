...
import { useMemo, useState, type ButtonHTMLAttributes, type HTMLAttributes } from 'react'
import { AlertTriangle, Ban, Check, ChevronRight, CircleSlash, Image as ImageIcon, Info, Layers, Loader2, RotateCcw, Shield, Type, X } from 'lucide-react'
import { getAsset, isRenderable, type AssetKind } from '@/lib/asset-manifest'
import { type ShellStateId } from '@/lib/shell-catalog'
import { OUTCOME_LABELS, type ShellIntentState } from '@/lib/shell-intent'

/* ------------------------------------------------------------------ */
/* Shared game interaction language                                   */
/* ------------------------------------------------------------------ */

export type GameTone = 'neutral' | 'primary' | 'danger'
export type GamePhase = 'idle' | 'pending' | 'accepted' | 'rejected' | 'timeout' | 'stale' | 'cancelled'

export function GameButton({ tone = 'neutral', phase = 'idle', leading, children, className = '', disabled, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { tone?: GameTone; phase?: GamePhase; leading?: React.ReactNode }) {
  const busy = phase === 'pending'
  return (
    <button className={`game-button is-${tone} is-${phase} ${className}`} disabled={disabled || busy} aria-busy={busy || undefined} {...props}>
      <span className="game-button-rail" aria-hidden="true" />
      <span className="game-button-icon" aria-hidden="true">{busy ? <Loader2 className="is-spin" /> : leading}</span>
      <span className="game-button-label">{children}</span>
      <ChevronRight className="game-button-chevron" aria-hidden="true" />
    </button>
  )
}

export function GamePanel({ kicker, title, children, className = '', ...props }: HTMLAttributes<HTMLElement> & { kicker?: string; title?: string }) {
  return (
    <section className={`game-panel ${className}`} {...props}>
      <span className="game-panel-corner is-a" aria-hidden="true" />
      <span className="game-panel-corner is-b" aria-hidden="true" />
      {(kicker || title) && <header className="game-panel-header">{kicker && <span>{kicker}</span>}{title && <h2>{title}</h2>}</header>}
      {children}
    </section>
  )
}

export function GameOverlay({ children, className = '', ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div className={`game-overlay ${className}`} {...props}><div className="game-overlay-freeze" aria-hidden="true" />{children}</div>
}

export function GameStatusBanner({ phase, title, children, className = '' }: { phase: GamePhase; title: string; children?: React.ReactNode; className?: string }) {
  const failure = phase === 'rejected' || phase === 'timeout' || phase === 'stale'
  return <div className={`game-status is-${phase} ${className}`} role={failure ? 'alert' : 'status'} aria-live={failure ? 'assertive' : 'polite'}><span className="game-status-pip" aria-hidden="true" /><span><b>{title}</b>{children && <small>{children}</small>}</span></div>
}
