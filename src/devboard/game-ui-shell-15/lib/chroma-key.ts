...
import { mockAssetAdapter as assetAdapter } from '@/lib/shell-adapters'
...
/**
 * 三段流水线的 fetch+decode 段。
 *
 * V0-11 变更：字节获取不再是本文件里的裸 `fetch`。它是壳层里唯一一处网络形状的调用，
 * 而当时它没有 requestId、没有超时、不能取消、也没有声明失败回退——这正是第三轮要清掉的
 * 「散落的半联网逻辑」。现在它走 `mockAssetAdapter.openBytes`：统一 requestId、统一超时、
 * 可取消，失败返回结构化 outcome 而不是抛异常。
 *
 * 失败链保持不变且完整：openBytes 非 loaded → Image 生命周期 → 最外层 catch 回落原始 src
 * （背板可见，但页面链路不断）。
 */
async function decodeSource(src: string): Promise<CanvasImageSource & { width: number; height: number }> {
  ...
  if (typeof createImageBitmap === 'function') {
    const bytes = await assetAdapter.openBytes(src, { fallback: 'static-fallback' })
    if (bytes.outcome === 'loaded' && bytes.blob) {
      try {
        return await createImageBitmap(bytes.blob)
      } catch {
        // 解码失败：落到 Image 路径
      }
    }
