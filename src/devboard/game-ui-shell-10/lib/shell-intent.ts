...
export type ShellIntentOutcome = 'accepted' | 'rejected' | 'stale' | 'timeout' | 'cancelled'
...
const OUTCOME_MESSAGES: Record<ShellIntentOutcome, string> = {
  accepted: '宿主已确认该请求。',
...
const OUTCOME_REASONS: Record<Exclude<ShellIntentOutcome, 'accepted'>, string> = {
...
export const OUTCOME_LABELS: Record<ShellIntentPhase, string> = {
  ...
  rejected: '已拒绝', stale: '版本过期', timeout: '请求超时', cancelled: '已取消',
}
