...
const FORCED_OUTCOMES: ForcedOutcome[] = ['auto', 'accepted', 'rejected', 'stale', 'timeout', 'cancelled']
const FORCED_LABELS: Record<ForcedOutcome, string> = {
  auto: 'AUTO（默认确认）', accepted: 'ACCEPTED', rejected: 'REJECTED', stale: 'STALE', timeout: 'TIMEOUT', cancelled: 'CANCELLED',
}
...
function mountSurface(pageId: string, ctx: MountContext, extras: {
  ...
  switch (pageId) {
    ...
    case 'transition-dream':
      return <TransitionDream mode="enter-dream" onComplete={() => ctx.navigate('transition-battle-intro')} />
    case 'transition-battle-intro':
      return <TransitionBattleIntro onComplete={() => ctx.navigate('hud-main')} />
...
export default function ControlPanelMain() {
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <PageStateFrame
                ...
                mockBoundary={page.mockBoundary}
