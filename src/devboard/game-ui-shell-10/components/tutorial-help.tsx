...
export function TutorialHelp({
  ...
  const acknowledge = async () => {
    ...
    if (r.status === 'accepted') {
      ...
      onAcknowledged?.()
      setMode('location')
    } else setLive(`确认未生效：${r.reason ?? r.status}`)
  }
  const replay = async () => {
        ...
        {mode === 'tutorial' && (
          ...
          >
            <div className="th-tut-head">
              <span className="th-tut-kicker"><GraduationCap size={12} /> 教程 <span className="mock-tag">MOCK</span></span>
        ...
        {mode === 'help' && (
          ...
          >
            <div className="th-help-head">
              <span className="th-tut-kicker"><CircleHelp size={12} /> 帮助 · 教程重放 <span className="mock-tag">MOCK</span></span>
