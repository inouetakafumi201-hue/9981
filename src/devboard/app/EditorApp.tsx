...
import { GameButton, GameInput, GameSelect, GameScrollArea } from '../components/index.js';
import './editor.css';
...
export function EditorApp(): JSX.Element {
  ...
  /** 图层的节点归属通过 `MapNode.layerId`（契约扩展位）——devboard 内部仍以 floor 作编辑轴，画布裁剪按楼层就近归类。 */
  ...
  return <div className="editor-app" style={cssVars} tabIndex={0} onKeyDown={handleKey} onKeyUp={handleKeyUp}>
    <header className="topbar">
      ...
      <div className="map-title"><GameInput aria-label="地图名称" value={map.name} onChange={(event) => changeMap(() => ({ ...map, name: event.target.value }))} /><span>{map.id}</span></div>
      <div className="top-actions">
        <GameButton variant="icon" title="撤销 (Ctrl+Z)" onClick={doUndo}><Undo2 size={16} /></GameButton>
        <GameButton variant="icon" title="重做 (Ctrl+Shift+Z / Ctrl+Y)" onClick={doRedo}><Redo2 size={16} /></GameButton>
        <GameButton variant="icon" title="新建空地图" onClick={() => { changeMap(() => blankMap(), '已清空新建。'); setSelection(null); }}>＋</GameButton>
        <GameButton variant="ghost" onClick={() => { const name = `${map.name} 副本`; changeMap(() => blueprintCopy(map, name), '已从当前地图创建蓝本副本。'); }}>蓝本新建</GameButton>
        <GameButton variant="primary" onClick={exportMap}>校验并导出</GameButton>
      </div>
    </header>
    <div className="workspace">
      ...
      <main className="main-area">
        <nav className="toolbar" aria-label="编辑工具">{([['select', '选择', 'V'], ['node', '放置场景', 'N'], ['edge', '拉边', 'E'], ['sample', '取样', 'I'], ['playtest', '测试运行', 'P']] as const).map(([id, label, shortcut]) => <GameButton key={id} variant="ghost" active={mode === id} className="tool-button" title={`${label} (${shortcut})`} onClick={() => changeMode(id)}><b>{shortcut}</b><span>{label}</span></GameButton>)}</nav>
      ...
      {rightPanelOpen ? <aside className="right-panel">
        ...
        <section className="materials"><div className="panel-heading"><h2>素材</h2><GameButton variant="ghost" onClick={() => setPaletteOpen(!paletteOpen)}>{paletteOpen ? '收起' : '展开 70 格'}</GameButton></div>{paletteOpen && <GameInput className="search" placeholder="搜索素材" value={materialQuery} onChange={(event) => setMaterialQuery(event.target.value)} />}<div className={`material-grid ${paletteOpen ? 'expanded' : ''}`}>{shownMaterials.slice(0, paletteOpen ? 70 : 7).map(([id, name, category], index) => <div key={id} draggable onDragStart={(event) => event.dataTransfer.setData('text/plain', id)} className={`material-card variant-${index % 5}`} title={`拖入画布放置 ${name}`}><i>{name[0]}</i><span>{name}</span><small>{category}</small></div>)}</div><p className="material-hint">开发者权限：全部已注册素材可用。拖入画布会挂接到最近场景。</p></section>
        <section className="playtest"><div className="panel-heading"><h2>Playtest</h2><GameButton variant="primary" onClick={playtest}>运行</GameButton></div><p>{playtestResult}</p><div className="topology"><span>连通组 {connectedGroups(map).length}</span><span>邻接节点 {adjacencyOf(map).size}</span></div><small>编译验证 MapData 结构与拓扑；几何和开发板图层不会进入 PrefabDef。</small></section>
      </aside> : null}
    </div>
  </div>;
}
...
function NodeInspector({ node, setFloor, update }: { node: MapNode; setFloor: (floor: number) => void; update: (patch: Partial<MapNode>) => void }): JSX.Element {
  return <div className="field-grid"><label>名称<GameInput value={node.name ?? ''} onChange={(event) => update({ name: event.target.value })} /></label><label>尺度<GameSelect value={node.scale} onChange={(value) => update({ scale: value as SceneScale })} options={nodeScales.map((scale) => ({ value: scale, label: scale }))} /></label><label>X<GameInput type="number" min="0" max="1" step="0.01" value={node.at.x} onChange={(event) => update({ at: { ...node.at, x: Number(event.target.value) } })} /></label><label>Y<GameInput type="number" min="0" max="1" step="0.01" value={node.at.y} onChange={(event) => update({ at: { ...node.at, y: Number(event.target.value) } })} /></label><label>楼层<GameInput type="number" value={node.floor} onChange={(event) => setFloor(Number(event.target.value))} /></label><label>Def<GameInput value={node.def} onChange={(event) => update({ def: event.target.value })} /></label></div>;
}
...
function EdgeInspector({ edge, update, setVisual, setPhysical, rotateVisual, rotatePhysical, clearVisual, clearPhysical, setAnchor, setWindow, moveWindow }: { edge: MapEdge; update: (patch: Partial<MapEdge>) => void; setVisual: () => void; setPhysical: () => void; rotateVisual: (deg: number) => void; rotatePhysical: (deg: number) => void; clearVisual: () => void; clearPhysical: () => void; setAnchor: (anchor: 'high' | 'low' | 'neutral') => void; setWindow: (enabled: boolean) => void; moveWindow: (to: { x: number; y: number }) => void }): JSX.Element {
  ...
  return <div className="field-grid">
    <label>方向<GameSelect value={edge.directionality} onChange={(value) => update({ directionality: value as MapEdge['directionality'] })} options={directions.map((direction) => ({ value: direction, label: direction }))} /></label>
    <label>Def<GameInput value={edge.def} onChange={(event) => update({ def: event.target.value })} /></label>
    <label>语义锚点<GameSelect value={edge.semanticAnchor ?? 'neutral'} onChange={(value) => setAnchor(value as 'high' | 'low' | 'neutral')} options={[{ value: 'high', label: '高地' }, { value: 'low', label: '洼地' }, { value: 'neutral', label: '中性' }]} /></label>
    ...
    {edge.transitionWindow?.control[0] ? (() => { const w = edge.transitionWindow!.control[0]!; return (
      <div className="transition-window-fields"><span className="small-title">过渡窗口位置（画布上拖图形亦可）</span>
        <label>X<GameInput type="number" min="0" max="1" step="0.01" value={Number(w.x.toFixed(3))} onChange={(event) => moveWindow({ x: Math.max(0, Math.min(1, Number(event.target.value))), y: w.y })} /></label>
