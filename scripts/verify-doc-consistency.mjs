...
function read(relPath) {
  let abs = resolve(ROOT, relPath);
  if (!existsSync(abs)) {
    const archiveRelPath = relPath.replace('.kiro/specs/', '.kiro/specs/_归档/2026-09-01/');
    const archiveAbs = resolve(ROOT, archiveRelPath);
...
console.log('\n===== 6.5 组件标准正面斜投影视角守卫 =====');
{
  ...
  const missing = requiredTargets.filter(
    (target) => !read(target).toLowerCase().includes('front-top axonometric'),
  );
  for (const target of missing) {
    console.log(`  ✗ ${target} 缺少固定口令 front-top axonometric view`);
  }

  const generatedPromptSource = read('.agents/skills/sprite-forge/tools/sprite-component.py').toLowerCase();
  const forbiddenExamples = ['cabinet projection', 'cavalier projection', 'among us'];
  const leakedExamples = forbiddenExamples.filter((example) => generatedPromptSource.includes(example));
  for (const example of leakedExamples) {
    console.log(`  ✗ sprite-component.py 的生成提示词依赖第三方案例: ${example}`);
  }

  if (missing.length || leakedExamples.length) {
    fail = 1;
  } else {
    console.log('  ✓ 组件正面斜投影口令完整，且未依赖第三方游戏或专有投影案例');
  }
}
