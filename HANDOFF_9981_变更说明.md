...
10. 建筑组（BuildingGroup）设计已永久弃用：不再进入 MapData JSON 契约、编辑器状态/UI、桥接、序列化或创作文档实施规范。旧 JSON 含 `buildingGroups` 时由导入器明确拒绝，并返回 `MAP_BUILDING_GROUP_DEPRECATED`。
11. 批量生成前必须完成玩法设计、机制实现、无头属性测试和大量用例测试，确认稳定后才生成、处理、抽验和挂载。
