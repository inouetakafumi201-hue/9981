 ...
 * 流程：
 *  1. 备份 src/
 *  2. 先确认基线全绿——基线不绿则任何击杀都无意义
 *  3. 逐个注入变异体，跑收缩后的套件；非零退出即记为"杀死"
 *     （包括超时、栈溢出、编译失败——这些都是"测试发现了问题"）
 ...
 * 本包没有本地 node_modules/vitest，实际由上级目录提供，
 * 因此不能硬编码路径。
 */
function resolveVitestBin(): string {
  ...
  for (;;) {
    ...
    if (parent === dir) {
      throw new Error('未找到 vitest 入口；请确认依赖已安装');
    }
    dir = parent;
  }
}
...
function runSuite(): { ok: boolean; detail: string } {
  // 直接调用 vitest 的 JS 入口，避免 Windows 上 spawnSync 对 .cmd 报 EINVAL，
  // 也避免 shell: true 带来的参数转义问题。
  const result = spawnSync(
    ...
    [VITEST_BIN, 'run', '--config', 'vitest.mutation.config.ts'],
    {
      cwd: ROOT,
      encoding: 'utf8',
      env: { ...process.env, L4_RUNS: process.env.MUTATION_RUNS ?? '400' },
      timeout: 180_000,
      shell: false,
    },
  );
...
function main(): number {
    ...
    for (const mutant of MUTANTS) {
      ...
      if (!applied.applied) {
        outcomes.push({
          ...
          desc: mutant.desc,
          status: 'invalid',
          expectEquivalent: Boolean(mutant.expectEquivalent),
    ...
    JSON.stringify(
      {
        ...
        expectEquivalent: equivalents.length,
  ...
  if (survivors.length > 0) {
    ...
    for (const survivor of survivors) {
      process.stdout.write(`  ${survivor.id}  ${survivor.desc}\n`);
    }
  }
  for (const equivalent of equivalents) {
    if (equivalent.status === 'killed') {
      process.stdout.write(`\n注意：${equivalent.id} 标注为预期等价却被杀死，标注有误。\n`);
    }
  }
