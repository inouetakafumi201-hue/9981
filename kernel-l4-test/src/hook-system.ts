...
export class HookSystem {
  private readonly hooks = new Map<string, Array<{ hook: HookDef; order: number }>>();
  private readonly defaultHandlers = new Map<string, (ctx: Context) => unknown>();
  private readonly callStack: Array<{ type: string; hookId: string }> = [];
  private reactionQueue: PendingReaction[] = [];
  private registrationOrder = 0;
  private depth = 0;
  private reactionRounds = 0;
   ...
   */
  snapshot(): {
    hooks: Array<{ on: string; entries: Array<{ id: string; phase: string; priority: number; order: number }> }>;
   ...
   */
  checkInvariants(idle = true): string[] {
    ...
    if (this.depth < 0) {
      push(HookInvariant.DEPTH_NEGATIVE, `depth=${this.depth}`);
    }
    if (this.depth > this.MAX_DEPTH) {
      push(HookInvariant.DEPTH_OVERFLOW, `depth=${this.depth} > ${this.MAX_DEPTH}`);
    }
    if (this.reactionRounds > this.MAX_REACTION_ROUNDS) {
      push(
        HookInvariant.ROUNDS_OVERFLOW,
        `reactionRounds=${this.reactionRounds} > ${this.MAX_REACTION_ROUNDS}`,
      );
    }
    // depth 为负时该比较必然成立，属 DEPTH_NEGATIVE 的派生噪声，不单独报。
    if (this.depth >= 0 && this.callStack.length > this.depth) {
      push(
        HookInvariant.STACK_DEPTH_MISMATCH,
        `callStack=${this.callStack.length} > depth=${this.depth}`,
      );
    }
    if (idle) {
      if (this.depth !== 0) push(HookInvariant.DEPTH_NOT_RESET, `depth=${this.depth}`);
      if (this.callStack.length !== 0) {
        push(
          HookInvariant.STACK_LEAKED,
          `callStack=[${this.callStack.map((f) => `${f.type}/${f.hookId}`).join(',')}]`,
        );
      }
      if (this.reactionQueue.length !== 0) {
        push(HookInvariant.QUEUE_LEAKED, `queue=${this.reactionQueue.length}`);
      }
    }
    ...
    for (const [on, entries] of this.hooks) {
      if (entries.length === 0) {
        push(HookInvariant.BUCKET_EMPTY, `event ${on} has an empty bucket`);
      }
      let prevOrder = -1;
      for (const entry of entries) {
        ...
        if (entry.hook.on !== on) {
          push(
            HookInvariant.BUCKET_MISKEYED,
            `hook ${entry.hook.id} has on=${entry.hook.on} but sits in bucket ${on}`,
          );
        }
        if (!VALID_PHASES.has(entry.hook.phase)) {
          push(HookInvariant.PHASE_INVALID, `hook ${entry.hook.id} phase=${String(entry.hook.phase)}`);
        }
        if (entry.order <= prevOrder) {
          push(
            HookInvariant.ORDER_NOT_MONOTONIC,
            `bucket ${on}: order ${entry.order} follows ${prevOrder}`,
          );
        }
        prevOrder = entry.order;
        if (seenOrders.has(entry.order)) {
          push(HookInvariant.ORDER_DUPLICATE, `order ${entry.order} appears more than once`);
        }
    ...
    if (total !== this.registrationOrder) {
      push(
        HookInvariant.ORDER_COUNT_MISMATCH,
        `registered=${total} but registrationOrder=${this.registrationOrder}`,
      );
    }
  ...
  collectAndSortInstead(type: string, data: any = {}): HookDef[] {
    ...
    return this.sortInsteadHooks(this.collectHooks(type, ctx, HookPhase.Instead))
  ...
  private createContext(type: string, data: any): Context {
  ...
  private resolveEntity(data: any): Entity | undefined {
    return data?.target ?? data?.entity;
  }

  private drainReactions(): void {
  ...
  private runPhase(phase: HookPhase, type: string, ctx: Context): void {
    for (const candidate of this.collectHooks(type, ctx, phase)) {
      this.invokeHook(type, candidate.hook, ctx);
    }
  }

  private runInsteadPhase(type: string, ctx: Context): boolean {
    const candidates = this.sortInsteadHooks(
      this.collectHooks(type, ctx, HookPhase.Instead),
    );

    for (const candidate of candidates) {
      const result = this.invokeHook(type, candidate.hook, ctx);
      if (result?.preventAll) return true;
      if (result?.preventExcept && !result.preventExcept.includes(type)) return true;
    }
    return false;
  }

  private invokeHook(type: string, hook: HookDef, ctx: Context): void | PreventResult {
    if (this.isReentry(type, hook.id)) throw new HookReentryError();
  ...
  private runDefault(type: string, ctx: Context): void {
  ...
  private collectHooks(type: string, ctx: Context, phase: HookPhase): HookCandidate[] {
    const candidates: HookCandidate[] = [];

    for (const entry of this.hooks.get(type) ?? []) {
      if (entry.hook.phase !== phase) continue;
      if (entry.hook.when && !entry.hook.when(ctx)) continue;
      candidates.push({
        hook: entry.hook,
        meta: {
          containerIndex: 0,
    ...
    for (let containerIndex = 0; containerIndex < entity.containerOrder.length; containerIndex++) {
      ...
      for (let slotIndex = 0; slotIndex < container.items.length; slotIndex++) {
        ...
        for (let ruleIndex = 0; ruleIndex < item.rules.length; ruleIndex++) {
          ...
          if (hook.on !== type || hook.phase !== phase) continue;
  ...
  private sortInsteadHooks(candidates: HookCandidate[]): HookCandidate[] {
  ...
  private isReentry(type: string, hookId: string): boolean {
    return this.callStack.some(
      (frame) => frame.type === type && frame.hookId === hookId,
    );
  }
}
