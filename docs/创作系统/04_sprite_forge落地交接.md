# sprite-forge 落地实现交接（可选历史开发工具）

> **现行定位（2026-09-05）**：本文档记录「把上游开源 2D 精灵管线引进本项目」的历史实现。V0 上的默认素材生成入口已经改为 Agent 调用 V0 `GenerateImage` 等平台工具；sprite-forge 与外部 API 适配器只保留为可选开发工具，不得成为 V0 原生管线的必需依赖。其产物若进入项目，仍必须通过统一的稳定登记、表现层实例化和编辑器可选验收，详见 `03_素材工作流管线实现交接.md`。
>
> 交接方 = 本项目开发期素材工具链。产物全部落在 `.agents/skills/sprite-forge/`，与产品和 `src/` 无关。
