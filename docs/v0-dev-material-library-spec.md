...
- 星标置顶常用素材
- 分类检索（全部 / AI 单位 / NPC / 载具 / 容器 / 物品 / 机关装置 / 装饰 / 过渡场景）
- 查看素材详情（挂载词条、品级、弱点）
...
- 下方**逻辑类别筛选**：`AI 单位` / `NPC` / `载具` / `容器` / `物品` / `机关装置` / `装饰` / `过渡场景`（与中央网格联动）。
- 当前选中项 = 青左条 + 淡青晕；星标素材在同筛选栏内排首。
- 筛选与搜索共用统一八类契约（与编辑器快捷库一致，数据共享）；旧六类只在导入边界迁移。
- 卡片和详情须显示放置语义：`原生逻辑`、`仅表现` 或 `绑定连线`。
...
<MaterialLibraryApp>（全屏根容器；挂载三界面切换回调）
  ...
  ├─ <FilterSidebar>
  │    ├─ <ScopeFilter>（全部 / 我的素材）
  │    └─ <CategoryFilter>（全部 / AI 单位 / NPC / 载具 / 容器 / 物品 / 机关装置 / 装饰 / 过渡场景）
  ├─ <LibraryTabs>
  │    ├─ <ElementTab>（可放置元素）
